...
import { useUpdateKandidatMutation } from '@/store/api/pegawaiApi'
...
export default function Form({ data }) {
  ...
  const [updateKandidat] = useUpdateKandidatMutation()
  ...
  const onSubmit = (value) => {
    updateKandidat({ data: value, id_divisi: id, id_kandidat: slug })
      .unwrap()
      .then((payload) => {
        if (payload.status) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className='mt-6 flex justify-end gap-4'>
        ...
        <Button type='submit'>Edit Kandidat</Button>
      </div>
    </form>
  )
}
